...
/** Build a workbook buffer + suggested filename for a single docente. */
async function buildAgendaBuffer(
  item: BatchAgendaItem,
  semesterLabel: string
): Promise<{ buffer: ArrayBuffer; filename: string }> {
  const wb = new ExcelJS.Workbook();
  wb.creator = "UCP Agenda";
  wb.created = new Date();
  // Reuse the public function indirectly by calling it on a captured workbook
  // is impractical; instead, we stream the existing logic by invoking the
  // single export that writes to disk would download. To avoid duplicating
  // ~450 lines we leverage the fact that exportAgendaToExcel both builds AND
  // downloads — so for batch we wrap the download by intercepting `a.click`.
  // Simpler approach: reimplement the save tail by producing the buffer here
  // via a temporary anchor capture would be fragile. Instead expose a builder.
  throw new Error("buildAgendaBuffer must be implemented via builder; see exportAgendasBatch");
}

/**
 * Download one or many agendas. With 1 item it produces a single .xlsx;
 * with 2+ items it bundles them into a .zip.
 *
 * Implementation note: to avoid duplicating the 450-line workbook builder,
 * we monkey-patch the download step inside exportAgendaToExcel by capturing
 * the produced Blob through a transient anchor click handler.
 */
export async function exportAgendasBatch(
  ...
  // Capture the blob produced by exportAgendaToExcel without triggering the
  // user-facing download. We override URL.createObjectURL + anchor.click for
  ...
  async function captureWorkbook(item: BatchAgendaItem) {
    ...
    document.body.appendChild = ((node: any) => {
      if (node && node.tagName === "A" && captured) {
        ...
        // Prevent real navigation by neutering click + remove
        node.click = () => {};
        // Return a detached node so removeChild works without DOM mutation
        return node;
      }
  ...
  if (items.length === 1) {
    // Plain single download — let the original function handle everything.
    await exportAgendaToExcel({
      user: items[0].user,
